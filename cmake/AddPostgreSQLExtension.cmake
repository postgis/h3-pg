...
function(PostgreSQL_add_extension LIBRARY_NAME)
  ...
  foreach(file ${EXTENSION_UPDATES})
    ...
    if(UPDATE_OUTPUT_NAME MATCHES "--unreleased\\.sql$" AND NOT EXTENSION_VERSION STREQUAL "unreleased")
      ...
      string(REGEX REPLACE "'unreleased'" "'${EXTENSION_VERSION}'" UPDATE_CONTENTS "${UPDATE_CONTENTS}")
    endif()

    if(NOT EXTENSION_VERSION STREQUAL "unreleased" AND NOT EXTENSION_UNRELEASED_TRANSITION_EMITTED)
      set(UPDATE_TARGET_SUFFIX "--${EXTENSION_VERSION}.sql")
      string(LENGTH "${UPDATE_OUTPUT_NAME}" UPDATE_OUTPUT_NAME_LENGTH)
      string(LENGTH "${UPDATE_TARGET_SUFFIX}" UPDATE_TARGET_SUFFIX_LENGTH)
      if(UPDATE_OUTPUT_NAME_LENGTH GREATER_EQUAL UPDATE_TARGET_SUFFIX_LENGTH)
        math(EXPR UPDATE_TARGET_SUFFIX_START "${UPDATE_OUTPUT_NAME_LENGTH} - ${UPDATE_TARGET_SUFFIX_LENGTH}")
        string(SUBSTRING "${UPDATE_OUTPUT_NAME}" ${UPDATE_TARGET_SUFFIX_START} ${UPDATE_TARGET_SUFFIX_LENGTH} UPDATE_OUTPUT_SUFFIX)
        if(UPDATE_OUTPUT_SUFFIX STREQUAL "${UPDATE_TARGET_SUFFIX}")
          ...
          set(UPDATE_TRANSITION_NAME "${EXTENSION_NAME}--unreleased--${EXTENSION_VERSION}.sql")
...
\echo Use "ALTER EXTENSION @EXTENSION_NAME@ UPDATE TO '@EXTENSION_VERSION@'" to load this file. \quit
          ...
          string(REPLACE "@SOURCE_VERSION@" "unreleased" UPDATE_TRANSITION_CONTENT "${UPDATE_TRANSITION_CONTENT}")
      ...
      if(UPDATE_OUTPUT_NAME_LENGTH GREATER_EQUAL UPDATE_TARGET_SUFFIX_LENGTH)
        ...
        if(UPDATE_OUTPUT_SUFFIX STREQUAL "${UPDATE_TARGET_SUFFIX}")
          if(NOT EXTENSION_PROJECT_DEV_RELEASE_TRANSITION_EMITTED)
            ...
            set(UPDATE_DEV_VERSION "${PROJECT_VERSION}dev")
            set(UPDATE_TRANSITION_NAME "${EXTENSION_NAME}--${UPDATE_DEV_VERSION}--${EXTENSION_VERSION}.sql")
            set(UPDATE_TRANSITION_OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/${UPDATE_TRANSITION_NAME}")
            set(UPDATE_TRANSITION_CONTENT [=[
-- Generated transition from @SOURCE_VERSION@ development installs.
\echo Use "ALTER EXTENSION @EXTENSION_NAME@ UPDATE TO '@EXTENSION_VERSION@'" to load this file. \quit
]=])
            string(REPLACE "@SOURCE_VERSION@" "${UPDATE_DEV_VERSION}" UPDATE_TRANSITION_CONTENT "${UPDATE_TRANSITION_CONTENT}")
            string(REPLACE "@EXTENSION_NAME@" "${EXTENSION_NAME}" UPDATE_TRANSITION_CONTENT "${UPDATE_TRANSITION_CONTENT}")
            string(REPLACE "@EXTENSION_VERSION@" "${EXTENSION_VERSION}" UPDATE_TRANSITION_CONTENT "${UPDATE_TRANSITION_CONTENT}")
            file(WRITE "${UPDATE_TRANSITION_OUTPUT}" "${UPDATE_TRANSITION_CONTENT}")
            list(APPEND UPDATE_TRANSITION_NAMES "${UPDATE_TRANSITION_NAME}")
          ...
          string(SUBSTRING "${UPDATE_OUTPUT_NAME}" 0 ${UPDATE_TARGET_SUFFIX_START} UPDATE_SOURCE_PREFIX)
          set(UPDATE_NAME_PREFIX "${EXTENSION_NAME}--")
          string(LENGTH "${UPDATE_NAME_PREFIX}" UPDATE_NAME_PREFIX_LENGTH)
          string(LENGTH "${UPDATE_SOURCE_PREFIX}" UPDATE_SOURCE_PREFIX_LENGTH)
          if(UPDATE_SOURCE_PREFIX_LENGTH GREATER UPDATE_NAME_PREFIX_LENGTH)
            string(SUBSTRING "${UPDATE_SOURCE_PREFIX}" 0 ${UPDATE_NAME_PREFIX_LENGTH} UPDATE_SOURCE_NAME_PREFIX)
            string(SUBSTRING "${UPDATE_SOURCE_PREFIX}" ${UPDATE_NAME_PREFIX_LENGTH} -1 UPDATE_SOURCE_VERSION)
            if(UPDATE_SOURCE_NAME_PREFIX STREQUAL "${UPDATE_NAME_PREFIX}" AND NOT UPDATE_SOURCE_VERSION STREQUAL "unreleased")
              ...
              if(NOT UPDATE_SOURCE_DEV_VERSION STREQUAL "${PROJECT_VERSION}dev" AND NOT EXTENSION_SOURCE_DEV_RELEASE_TRANSITION_EMITTED)
                ...
                set(UPDATE_TRANSITION_NAME "${EXTENSION_NAME}--${UPDATE_SOURCE_DEV_VERSION}--${EXTENSION_VERSION}.sql")
                set(UPDATE_TRANSITION_OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/${UPDATE_TRANSITION_NAME}")
                set(UPDATE_TRANSITION_CONTENT [=[
-- Generated transition from @SOURCE_VERSION@ development installs.
\echo Use "ALTER EXTENSION @EXTENSION_NAME@ UPDATE TO '@EXTENSION_VERSION@'" to load this file. \quit
]=])
                string(REPLACE "@SOURCE_VERSION@" "${UPDATE_SOURCE_DEV_VERSION}" UPDATE_TRANSITION_CONTENT "${UPDATE_TRANSITION_CONTENT}")
                string(REPLACE "@EXTENSION_NAME@" "${EXTENSION_NAME}" UPDATE_TRANSITION_CONTENT "${UPDATE_TRANSITION_CONTENT}")
                string(REPLACE "@EXTENSION_VERSION@" "${EXTENSION_VERSION}" UPDATE_TRANSITION_CONTENT "${UPDATE_TRANSITION_CONTENT}")
                file(WRITE "${UPDATE_TRANSITION_OUTPUT}" "${UPDATE_TRANSITION_CONTENT}")
                list(APPEND UPDATE_TRANSITION_NAMES "${UPDATE_TRANSITION_NAME}")
