...
WITH
    clip AS (
        SELECT
            h3,
            h3_raster_summary_stats_agg(stats ORDER BY id) AS stats
        FROM (
            -- id, h3, stats
    ...
    centroids AS (
        ...
        FROM (
            -- id, h3, stats
            SELECT r.id, (h3_raster_summary_centroids(r.rast, :resolution)).*
            FROM h3_test_rasters r
        ) t
        GROUP BY 1)
SELECT COUNT(*)
...
-- Class summary and summary stats (clip) should agree on per-cell pixel counts.
WITH
    class_clip AS (
        SELECT
            h3,
            sum((summary).count) AS count
        FROM (
            SELECT
                h3,
                val,
                h3_raster_class_summary_item_agg(summary ORDER BY id) AS summary
            FROM (
                -- id, h3, val, summary
                SELECT r.id, (h3_raster_class_summary_clip(r.rast, :resolution)).*
                FROM h3_test_rasters r
            ) t
            GROUP BY 1, 2
        ) t
...
SELECT COUNT(*)
FROM class_clip c FULL OUTER JOIN stats_clip s ON c.h3 = s.h3
